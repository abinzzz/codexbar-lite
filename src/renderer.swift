...
if args.count == 2 && args[1] == "--preferences" {
    print(NSWorkspace.shared.accessibilityDisplayShouldReduceMotion ? "reduce-motion" : "animate")
    exit(0)
}
guard args.count == 2 || (args.count == 3 && args[2] == "--batch") else { fail() }
let input = FileHandle.standardInput.readDataToEndOfFile()
let frames: [[Row]]
if args.count == 3 {
    guard let decoded = try? JSONDecoder().decode([[Row]].self, from: input),
          !decoded.isEmpty, decoded.count <= 61 else { fail() }
    frames = decoded
} else {
    guard let rows = try? JSONDecoder().decode([Row].self, from: input) else { fail() }
    frames = [rows]
}
guard frames.allSatisfy({ rows in
    rows.count == 2 && rows.allSatisfy { $0.label.count <= 12 && ($0.percent == nil || (0...100).contains($0.percent!)) }
}) else { fail() }
let dark = args[1].lowercased() == "dark"
for rows in frames {
